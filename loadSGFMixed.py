...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 30 11:36:43 2016

@author: nubby

See loadSGF.py for extensive comments. This file is identical but loads in games 
where a strong player plays a weak player. I only load in the strong players moves.

Using these kinds of files increased my go robot's playing strength quite a bit. 
It teaches the robot how to respond to silly moves and strange board positions 
that you only see in beginner games and don't see in the stronger players' games. But only 
using the half of the game where the strong player makes moves ensures that the 
bot is not trained to make the weak moves that weaker players do.

The boundaries of what constitutes weak and strong can be changed. Here I use stronger than 6 kyu for the
strong players and weaker than 16 kyu for the weak players. 
...
cwd = os.getcwd()
path = cwd + '/data_mixed'
pickleRoot = 'pickles_mixed/mixedPickleFile_'
            ...
            try:
                ...
                for play in matchPlay:
                   ...
                   #     print "move found: ", move[0]
                        ...
                        #Only add to features and labels if it's a white move
                        if (thisMoveColor == 'W'):
                            labels.append(label)
...
def loadFiles(pathName, parseType, rankingSystem):
    ...
    if (rankingSystem == "ELO" or rankingSystem == "Traditional"):
               ...
               try:
                   with open(file, 'r') as src:
                       ...
                       if (rankingSystem == "ELO"):
                           ...
                           if (bOK and wOK):
                               os.rename(file, pathName + "nine/nine_mixed/" + filename)
                           else:
                               os.rename(file,  pathName + "nine/nine_not_needed/" + filename)
                       else: 
                           ...
                           if ((bKyuDan == 'k' or bKyuDan == 'K') and bRank >= 16):
                               bOK = True
                               
                           if (wKyuDan == 'd' or wKyuDan == 'D' or wKyuDan == 'P' or wKyuDan == 'p' or wRank <= 6):
                               wOK = True
        ...
        print "Mixed 9x9 games found: ", countFiles(pathName + "nine/nine_mixed")
        print "Not needed 9x9 games found: ",  countFiles(pathName + "nine/nine_not_needed")
        ...
        for file in nine_files:
            ...
            if file.endswith(".sgf"):
               filename = os.path.basename(file)
               os.rename(file, pathName + "nine/nine_mixed/" + filename)
        print "Strong 9x9 games found: ",  countFiles(pathName + "nine/nine_mixed")
    ...
    nine_files_strong = glob.glob(pathName +  "nine/" + "nine_mixed/" +  "*.sgf")
